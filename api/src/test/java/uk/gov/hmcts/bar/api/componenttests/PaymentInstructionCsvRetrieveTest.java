...
public class PaymentInstructionCsvRetrieveTest extends ComponentTestBase {
	...
	public void givenPostalOrderPaymentInstructionDetails_retrieveAsCvs() throws Exception {
		PostalOrder proposedPostalOrderPaymentInstructionRequest = postalOrderPaymentInstructionRequestWith()
				.payerName("Mr Payer Payer").amount(533).currency("GBP").status("D").postalOrderNumber("000000")
				.build();

		restActions.post("/postal-orders", proposedPostalOrderPaymentInstructionRequest)
				.andExpect(status().isCreated());

		restActions.getCsv("/payment-instructions").andExpect(status().isOk()).andExpect(result -> {
			Assert.assertEquals(
					String.format("\"Daily sequential payment ID\"%s\"Date\"%s\"Payee name\"%s\"Cheque Amount\"%s"
							...
							+ "%s\"\"%s\"\"%s\"\"%s\"\"%s\"\"%s\"\"%s\"\"%s\"\"%s", SEPARATOR, SEPARATOR, SEPARATOR,
							SEPARATOR, SEPARATOR, SEPARATOR, SEPARATOR, SEPARATOR, SEPARATOR, SEPARATOR, SEPARATOR,
							SEPARATOR, EOL, SEPARATOR, CURRENT_DATE, SEPARATOR, SEPARATOR, SEPARATOR, SEPARATOR,
							SEPARATOR, SEPARATOR, SEPARATOR, SEPARATOR, SEPARATOR, SEPARATOR, SEPARATOR, EOL),
