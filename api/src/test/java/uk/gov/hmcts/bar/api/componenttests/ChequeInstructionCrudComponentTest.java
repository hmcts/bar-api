...
public class ChequeInstructionCrudComponentTest extends ComponentTestBase {
	...
	public void whenChequeInstructionDetails_thenCreateChequePaymentInstruction() throws Exception {
		Cheque proposedChequePaymentInstructionRequest = chequePaymentInstructionRequestWith()
				.payerName("Mr Payer Payer").amount(500).currency("GBP").status("D").chequeNumber("000000").build();
		...
		restActions.post("/cheques", proposedChequePaymentInstructionRequest).andExpect(status().isCreated())
				.andExpect(body().as(ChequePaymentInstruction.class, chequePaymentInstruction -> {
					assertThat(chequePaymentInstruction).isEqualToComparingOnlyGivenFields(
							chequePaymentInstructionWith().payerName("Mr Payer Payer").amount(500).currency("GBP")
									.status("D").chequeNumber("000000"));
				}));
	}
	...
	public void whenChequeInstructionWithInvalidChequeNumber_thenReturn400() throws Exception {
		Cheque proposedChequePaymentInstructionRequest = chequePaymentInstructionRequestWith()
				.payerName("Mr Payer Payer").amount(500).currency("GBP").status("D").chequeNumber("xxxxxx").build();

		restActions.post("/cheques", proposedChequePaymentInstructionRequest).andExpect(status().isBadRequest());
	}

	@Test
	public void whenChequeInstructionWithInvalidCurrency_thenReturn400() throws Exception {
		Cheque proposedChequePaymentInstructionRequest = chequePaymentInstructionRequestWith()
				.payerName("Mr Payer Payer").amount(500).currency("xxx").chequeNumber("000000").build();
	...
	public void whenChequePaymentInstructionIsSubmittedByPostClerk_expectStatus_200() throws Exception {
		...
		PaymentInstructionUpdateRequest statusUpdateRequest = paymentInstructionUpdateRequestWith().status("P").build();

		restActions.post("/cheques", proposedChequePaymentInstructionRequest).andExpect(status().isCreated());

		restActions.patch("/payment-instructions/1", statusUpdateRequest).andExpect(status().isOk());
	...
	public void whenNonExistingChequePaymentInstructionIsSubmittedByPostClerk_expectStatus_404() throws Exception {
		...
		restActions.patch("/payment-instructions/1000", statusUpdateRequest).andExpect(status().isNotFound());
	...
	public void whenInvalidCaseReferenceForAChequePaymentInstructionIsCreated_expectStatus_201() throws Exception {
		...
		CaseReference caseReference = caseReferenceWith().caseReference("?????^^^^11").build();
		...
		restActions.get("/payment-instructions?payerName=Mr Payer Payer").andExpect(status().isOk())
				.andExpect(body().as(List.class, chequePaymentInstructionList -> {
					assertThat(chequePaymentInstructionList.get(0)).isEqualToComparingOnlyGivenFields(
							chequePaymentInstructionWith().payerName("Mr Updated Payer").amount(600).currency("GBP")
									.status("D").chequeNumber("000000"));
				}));
	}

	@Test
	public void whenSearchNonExistingChequePaymentInstructionByPayerName_expectStatus_200AndEmptyList()
			throws Exception {
		Cheque proposedChequePaymentInstructionRequest = chequePaymentInstructionRequestWith()
				.payerName("Mr Payer Payer").amount(500).currency("GBP").status("D").chequeNumber("000000").build();
		...
		restActions.get("/payment-instructions?payerName=NonExisting").andExpect(status().isOk()).andExpect(body()
				.as(List.class, chequePaymentInstructionList -> assertTrue(chequePaymentInstructionList.isEmpty())));
	}
	...
	public void whenSearchChequePaymentInstructionByChequeNumber_expectStatus_200() throws Exception {
		...
		restActions.get("/payment-instructions?chequeNumber=000000").andExpect(status().isOk())
				.andExpect(body().as(List.class, chequePaymentInstructionList -> {
					assertThat(chequePaymentInstructionList.get(0)).isEqualToComparingOnlyGivenFields(
							chequePaymentInstructionWith().payerName("Mr Updated Payer").amount(600).currency("GBP")
									.status("D").chequeNumber("000000"));
	...
	@Test
	public void whenSearchNonExistingChequePaymentInstructionByChequeNumber_expectStatus_200AndEmptyList()
			throws Exception {
		Cheque proposedChequePaymentInstructionRequest = chequePaymentInstructionRequestWith()
				.payerName("Mr Payer Payer").amount(500).currency("GBP").status("D").chequeNumber("000000").build();
		...
		restActions.get("/payment-instructions?chequeNumber=111111").andExpect(status().isOk()).andExpect(body()
				.as(List.class, chequePaymentInstructionList -> assertTrue(chequePaymentInstructionList.isEmpty())));
	}
	...
	public void whenChequePaymentInstructionIsUpdated_expectStatus_200() throws Exception {
		...
		Cheque updatedChequePaymentInstructionRequest = chequePaymentInstructionRequestWith()
				.payerName("Mr Payer Payer").amount(6000).currency("GBP").status("D").chequeNumber("000000").build();

		restActions.post("/cheques", proposedChequePaymentInstructionRequest).andExpect(status().isCreated());

		restActions.put("/cheques/1", updatedChequePaymentInstructionRequest).andExpect(status().isOk());
	...
	public void whenNonExistingChequePaymentInstructionIsUpdated_expectStatus_404() throws Exception {
		...
		restActions.put("/cheques/1000", updatedChequePaymentInstructionRequest).andExpect(status().isNotFound());
