...
public interface PaymentInstructionStatusRepository
	...
	List<PaymentInstructionUserStats> getPaymentInstructionsByStatusByUserGroup(@Param("userRole") String userRole, @Param("status") String status);
	...
	@Query(name = "PIRejectedByDM", value = "SELECT new uk.gov.hmcts.bar.api.data.model.PaymentInstructionUserStats"
			...
			+ "pi.id IN (SELECT piinner.id FROM PaymentInstruction piinner WHERE piinner.status = :status) AND pi.id = pis.paymentInstructionStatusReferenceKey.paymentInstructionId "
			+ "AND pis.paymentInstructionStatusReferenceKey.status = 'A' AND pis.barUserId = bu.id GROUP BY bu.id")
	List<PaymentInstructionUserStats> getPaymentInstructionsRejectedByDM(@Param("status") String status);
