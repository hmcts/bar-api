provider "azurerm" {
  ...
  alias                      = "mi_vault"
  subscription_id            = local.sdp_environment_ids[local.bar_environment].subscription
}
...
locals {
  ...
  bar_environment = lookup(local.sdp_cft_environments_map, var.env, var.env)
...
module "sdp_db_user" {
  ...
  env    = local.bar_environment

  server_name       = "${var.product}-postgres-db-v15-${var.env}"
  server_fqdn       = module.bar-database-v15.fqdn
  server_admin_user = module.bar-database-v15.username
  server_admin_pass = module.bar-database-v15.password
